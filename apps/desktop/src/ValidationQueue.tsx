...
export const ValidationQueue: React.FC<ValidationQueueProps> = ({ onValidated }) => {
  ...
  const handleDecision = async (
    ...
    try {
      ...
      // Only show metrics that SUMO actually produced. When the re-simulation
      // returns nothing, say so honestly -- never fall back to invented numbers.
      if (data.sim_result) {
        setSimDelta(data.sim_result);
        setMessage(`Decision recorded: ${action} for ${data.edge_id}. Model & OpenDRIVE recompiled; simulation re-run.`);
      } else {
        setSimDelta(null);
        setMessage(
          `Decision recorded: ${action} for ${data.edge_id}. Model & OpenDRIVE recompiled. ` +
          `Simulation unavailable${data.sim_error ? ` (${data.sim_error})` : ""}.`
        );
      }
      ...
      {simDelta && (
        ...
        >
          <h4 style={{ margin: "0 0 8px 0", color: "#34d399", fontSize: "14px" }}>
            ✓ Traffic Metrics Regenerated by SUMO on the Accepted Network
          </h4>
          <p style={{ margin: "0 0 12px 0", fontSize: "12px", color: "#9ca3af" }}>
            Network recompiled with the accepted lane count and re-simulated (baseline arm, seed 42).
          </p>
          <div style={{ display: "flex", gap: "24px" }}>
            <div>
              <span style={{ fontSize: "11px", color: "#9ca3af" }}>Avg Travel Time</span>
              <div style={{ fontSize: "16px", fontWeight: 700, color: "#f3f4f6" }}>
                {typeof simDelta?.baseline?.avg_travel_time_s === "number"
                  ? `${simDelta.baseline.avg_travel_time_s.toFixed(1)}s`
                  : "N/A"}
              </div>
            </div>
            <div>
              ...
              <div style={{ fontSize: "16px", fontWeight: 700, color: "#f3f4f6" }}>
                {typeof simDelta?.baseline?.completed_vehicles === "number"
                  ? `${simDelta.baseline.completed_vehicles} veh`
                  : "N/A"}
              </div>
            </div>
            <div>
              <span style={{ fontSize: "11px", color: "#9ca3af" }}>Mean Queue</span>
              <div style={{ fontSize: "16px", fontWeight: 700, color: "#f3f4f6" }}>
                {typeof simDelta?.baseline?.mean_queue_length_m === "number"
                  ? `${simDelta.baseline.mean_queue_length_m.toFixed(1)}m`
                  : "N/A"}
              </div>
            </div>
          </div>
